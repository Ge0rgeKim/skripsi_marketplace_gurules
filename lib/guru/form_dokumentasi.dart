...
class _form_dokumentasiState extends State<form_dokumentasi> {
  ...
  Future savedata() async {
    ...
    var response =
        await add_dataTopUp(body, pickedfile_!.path, int.parse(data_sesi['id_guru']));
    if (response) {
      setState(() {
        pickedfile_ = null;
      });
  ...
  static Future<bool> add_dataTopUp(
    ...
    var request = http.MultipartRequest(
      'POST',
      Uri.parse("https://literasimilenial.net/george/public/api/dokumentasi"),
    )
      ..fields.addAll(body)
      ..headers.addAll(headers)
      ..files.add(await http.MultipartFile.fromPath('image', filepath));
  ...
  Future save_dokumentasi() async {
    if (keteranganDokumentasiController.text.isEmpty || pickedfile_ == null) {
      ...
      Navigator.push(
        ...
        MaterialPageRoute(
          builder: (context) {
            return daftar_dokumentasi(index_user: int.parse(data_sesi['id_guru']));
          },
        ),
      );
    }
